...
class Feature_ExclusionLoss(nn.Module):

    def __init__(self, level=3, level_begin=0):
        """
        Loss on the gradient. based on:
        http://openaccess.thecvf.com/content_cvpr_2018/papers/Zhang_Single_Image_Reflection_CVPR_2018_paper.pdf
        """
        super(Feature_ExclusionLoss, self).__init__()
        self.level = level
    ...
    def _all_comb_from_ZY(self, grad1_s, grad2_s):
        grad1_s_ = grad1_s ** 2
        grad2_s_ = grad2_s ** 2
        grad1_s_list = [grad1_s_] * self.channel
        grad2_s_list = [grad2_s_] * self.channel

        permute = list(range(self.channel))
        for i in range(self.channel):
            permute = self.rotate(permute)
            grad2_s_list[i] = grad2_s_list[i][:, permute, :, :]

        grad1_s_ = torch.cat(grad1_s_list, dim=1)
        grad2_s_ = torch.cat(grad2_s_list, dim=1)

        return torch.mean(grad1_s_ * grad2_s_) ** 0.2

    def forward(self, img1, img2):
        self.channel = img1.shape[1]
        gradx_loss, grady_loss = self.get_gradients(img1, img2)
        loss_gradxy = sum(gradx_loss) / (self.level * (self.channel ^ 2)) + sum(grady_loss) / (
                    self.level * (self.channel) ^ 2)
        return loss_gradxy / 2.0
